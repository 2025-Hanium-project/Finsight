...
class RiskAgent(ReportAnalysisAgent):
    """리스크 분석 에이전트 클래스"""
    ...
    async def process(self, report_contents: List[Any], target_type: str, target_name: str) -> Dict[str, Any]:
        """리스크 분석 처리 메인 함수"""
        return await analyze_risk(report_contents, target_type, target_name)


async def analyze_risk(report_contents: List[Any], target_type: str, target_name: str) -> Dict[str, Any]:
    """
    리포트의 리스크를 분석하는 에이전트
    """
    ...
    try:
        ...
        # 프롬프트 생성
        prompt = _create_risk_prompt(validated_contents, target_type, target_name)
        ...
        logger.log_completion("리스크 분석", processing_time, extra={
        ...
        processing_time = (datetime.now() - start_time).total_seconds()
        logger.log_error("리스크 분석", e, extra={
        ...
        raise AgentError(
            ...
            message=f"리스크 분석 중 오류 발생: {str(e)}",
...
def _create_risk_prompt(report_contents: List[Any], target_type: str, target_name: str) -> str:
    """리스크 분석 프롬프트 생성"""
    ...
    return f"""
너는 금융 리스크 분석 전문가이며, 반드시 아래 예시와 동일한 JSON만 반환하는 API 역할을 한다.
...
위 리포트들을 종합적으로 분석하여 {target_name}의 투자 리스크를 분석하라:
- 주요 리스크 요인들과 각각의 심각도, 발생 확률 - 한국어 필수
- 전체 리스크 점수 (0-100, 높을수록 위험)
- 리스크 레벨 (high/medium/low)
- 리스크 트렌드 (increasing/stable/decreasing)
- 리스크 완화 전략 - 한국어 필수
